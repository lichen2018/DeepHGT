...
train_length = 100
random_snp_count = 10
file_name = 'hgt_bkp_info_2.txt'
#onehot_input = encodeSequence(seq)

x_train = np.load('deepHGT_120_4_x_train.npy')
x_test = np.load('deepHGT_120_4_x_test.npy')
y_train = np.load('deepHGT_120_4_y_train.npy')
y_test = np.load('deepHGT_120_4_y_test.npy')
x_valid = np.load('deepHGT_120_4_x_valid.npy')
y_valid = np.load('deepHGT_120_4_y_valid.npy')

'''sample_list = []
for i in range(len(train_seq_list)):
    sample_list.append([train_seq_list[i],labels[i]])'''

model = get_classification_model(train_length)
h5_path = "deepHGT_100_8.4.2.h5"
...
plt.legend()
plt.savefig('loss_100_8.4.3.pdf')

to_h5_path = "deepHGT_100_8.4.3.h5"
